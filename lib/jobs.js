...
function createApiJob({
  ...
  const job = {
    ...
    auto_run_requested: auto_run_requested === true,
    policy_result: null,
    approval: {
      required: true,
      approved: false,
  ...
  if (job.auto_run_requested) {
    const policyResult = evaluateJobForAutoRun(job);
    job.policy_result = policyResult;

    if (policyResult.eligible) {
      job.status = "auto_running";
      job.approval.approved = true;
      job.approval.approved_by = `policy:${policyResult.policy_id}`;
      job.approval.approved_at = nowIso();
      job.approval.approval_note = policyResult.reason;
    }
  }
...
function isJobPolicyAutoRunnable(job) {
  return (
    job.auto_run_requested === true &&
    job.policy_result &&
    job.policy_result.eligible === true &&
    (job.status === "auto_running" || job.status === "approved")
  );
}
