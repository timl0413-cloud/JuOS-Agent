...
} = require("./workspace-target-registry");
...
const PROFILE_TASK_TYPES = {
  ...
  nova: ["inspect_only", "supervised_implement", "approved_finalize"],
  joa: ["inspect_only", "supervised_implement", "approved_finalize"],
  finance: ["supervised_implement", "approved_finalize"],
  ob: ["supervised_implement", "approved_finalize"],
  jucore: ["supervised_implement", "approved_finalize"],
  gsync: ["inspect_only", "supervised_implement", "approved_finalize"],
  spacea: ["inspect_only", "supervised_implement", "approved_finalize"],
  stuf: ["inspect_only", "supervised_implement", "approved_finalize"],
  ministry: ["inspect_only", "supervised_implement", "approved_finalize"],
};
...
const NOVA_REPO_REF = "Nova";
const NOVA_WORKSPACE_REF = "C:\\projects\\Nova";
const GSYNC_REPO_REF = "juos-knowledge-vault";
const GSYNC_WORKSPACE_REF = "C:\\projects\\juos-knowledge-vault";
const STUF_REPO_REF = "STUF";
const STUF_WORKSPACE_REF = "C:\\projects\\SpaceA\\STUF-Website";
const APPROVED_FINALIZE_CONTRACTS = {
  ...
  nova: {
    repo_ref: NOVA_REPO_REF,
    workspace_ref: NOVA_WORKSPACE_REF,
  },
  gsync: {
  ...
  stuf: {
    repo_ref: STUF_REPO_REF,
    workspace_ref: STUF_WORKSPACE_REF,
  },
};
...
function validateRepoRefForProfile(workerProfile, repoRef, options = {}) {
...
function validateApprovedFinalizeContract({
  ...
  if (isMultiWorkspaceProfile(workerProfile)) {
    if (!PROFILE_TASK_TYPES[workerProfile]?.includes("approved_finalize")) {
      errors.push(
    ...
    const resolvedWorkspace = resolveExecutionWorkspaceRef(
      workerProfile,
      workspaceRef
    );
    ...
    return {
      ok: errors.length === 0,
      errors,
      workspaceRef: resolvedWorkspace,
...
function validateWorkspaceRefForProfile(workerProfile, workspaceRef, options = {}) {
  if (isMultiWorkspaceProfile(workerProfile)) {
...
function validateCreateJobInput(input) {
  ...
  if (isMultiWorkspaceProfile(workerProfile)) {
    const multiError = validateMultiWorkspaceJob(workerProfile, input);
    if (multiError && !errors.includes(multiError)) {
      errors.push(multiError);
    }
  } else {
