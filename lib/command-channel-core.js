...
function validateRepoRefForProfile(workerProfile, repoRef) {
  ...
  if (workerProfile === "joa") {
    if (normalizeRepoRef(repoRef) !== normalizeRepoRef(JOA_REPO_REF)) {
      return `repo_ref must be "${JOA_REPO_REF}" for target_worker_profile "joa"`;
    }
    return null;
  }
...
function validateWorkspaceRefForProfile(workerProfile, workspaceRef) {
  ...
  if (!workspaceRef || typeof workspaceRef !== "string" || !workspaceRef.trim()) {
    return `workspace_ref is required for target_worker_profile "joa"`;
  }
  ...
  if (
    ...
    return `workspace_ref must be "${JOA_WORKSPACE_REF}" for target_worker_profile "joa"`;
  }
