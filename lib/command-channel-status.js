...
function buildRecoveryCommand(job, options = {}) {
  ...
  if (style === "node") {
    return `node scripts/command-channel-worker.js --http --once --worker-profile ${profile} --job-id ${jobId} --cursor-agent`;
  }

  return `.\\scripts\\start-juos-worker.ps1 -Profile ${profile} -JobId ${jobId}`;
}
