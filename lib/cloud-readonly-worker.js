...
function executeCloudReadonlyJob(job, context = {}) {
  ...
  if (
    ...
    job.target_worker_profile !== WORKER_PROFILE_ID
  ) {
    return buildFailureResult(job, [
      `target_worker_profile must be "${WORKER_PROFILE_ID}" or omitted`,
    ]);
  }
  ...
  const eligibility = evaluateWorkerEligibility(job, profile, routingContext);
