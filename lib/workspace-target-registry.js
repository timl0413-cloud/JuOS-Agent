...
const SCOPED_WRITE_PROFILES = ["gsync"];
...
function isScopedWriteProfile(workerProfile) {
  return SCOPED_WRITE_PROFILES.includes(workerProfile);
}

function getScopedWritePrefix(workerProfile) {
  const profile = getProfileConfig(workerProfile);
  if (!profile?.allowed_write_scope) {
    return null;
  }

  const scope = String(profile.allowed_write_scope)
    .trim()
    .replace(/\\/g, "/")
    .replace(/\/+\*\*?$/, "")
    .replace(/\/+$/, "");
...
function isPathUnderWriteScope(workerProfile, filePath) {
  ...
  const scopeRoot = prefix.replace(/\/+$/, "");
  return normalized === scopeRoot || normalized.startsWith(prefix);
}

function validateWriteScopePaths(workerProfile, filePaths = []) {
  if (!isScopedWriteProfile(workerProfile)) {
    return null;
  }

  const profile = getProfileConfig(workerProfile);
  const scope = profile?.allowed_write_scope || "gsync/**";
  const violations = filePaths
    .map((filePath) => normalizeRepoRelativePath(filePath))
    .filter((filePath) => filePath.length > 0)
    .filter((filePath) => !isPathUnderWriteScope(workerProfile, filePath));

  if (violations.length === 0) {
    return null;
  }

  return `paths outside allowed_write_scope "${scope}": ${violations.join(", ")}`;
}
