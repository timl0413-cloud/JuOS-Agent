...
function buildCurrentQueueFromManifest(manifest, summary, timeSummary, motionState, options = {}) {
  ...
  return {
    ...
    note: `${manifestNotes} · ${manifestPathLabel} · rows with a matching job show live command-channel status; unmatched rows stay planned/pending`,
...
function renderBatchTaskRows(entries, emptyText) {
  ...
  return entries
    .map((row) => {
      ...
      return `<tr class="batch-row batch-state-${escapeHtml(batchState)} ${sourceClass}" title="${escapeHtml(sourceHint)}">
...
function renderCurrentQueueBlock(currentQueue, isLive) {
  ...
  return `<section class="batch-section batch-section-primary">
    ...
    ${reviewHistoryNote}
    ${batchTable}
    <p class="batch-legend muted">Row colors: grey = completed · green = running · yellow = pending / needs action · red = failed / blocked · blue = future / not started · Status ending in “· live” = matched command-channel job · “· planned” = no live job yet</p>
  </section>`;
}
...
<head>
  ...
  <style>
    :root {
      ...
      --batch-completed: #6b7280; --batch-running: #34d399; --batch-pending: #fbbf24; --batch-failed: #f87171; --batch-future: #38bdf8; --batch-idle: #9aa8bc;
    ...
    * { box-sizing: border-box; }
    ...
    tr.batch-row.batch-state-running { background: rgba(52, 211, 153, 0.12); }
