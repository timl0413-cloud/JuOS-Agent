...
function renderLiveRunningStatusCell(row, nowMs = Date.now()) {
  const motionState = deriveRunningMotionState(row, nowMs);
  const label = escapeHtml(row.status_label || row.hint || "Ongoing · live");
...
const TOWER_EXECUTION_LABELS = {
  RUNNING: "Running — worker active",
  NO_WORKER: "No worker running — approved/planned work pending",
  WAITING_TIM: "Waiting for Tim — approval/action needed",
  ...
  return entries
    .map((row) => {
      ...
      const statusCell = isLiveRunning
        ? renderLiveRunningStatusCell(row, nowMs)
        : `<span class="batch-status-label">${escapeHtml(row.status_label || row.hint || row.status || "")}</span>`;
      ...
      return `<tr class="batch-row batch-state-${escapeHtml(batchState)} ${sourceClass}${motionClass}" title="${escapeHtml(sourceHint)}">
        ...
        <td class="batch-meta muted">${escapeHtml(row.profile)} · ${escapeHtml(row.repo)}${sourceMeta}</td>
      </tr>`;
    })
    .join("");
}
...
function renderSingleBatchTable(batchRows, emptyText) {
  return `<div class="card batch-table-primary">
    <table class="batch-task-table">
      <thead><tr><th>Task</th><th>Purpose</th><th>Job ID</th><th>Status</th><th>Tim action</th><th>Profile · Repo</th></tr></thead>
...
function renderTowerExecutionHeaderHtml(executionHeader) {
  ...
  if (executionHeader.last_completed?.title) {
    const when = executionHeader.last_completed.at_display
      ? ` · ${executionHeader.last_completed.at_display}`
      : "";
    contextParts.push(
      `<span class="tower-exec-context-item"><span class="tower-exec-context-label">Last done</span> ${escapeHtml(executionHeader.last_completed.title)}${escapeHtml(when)}</span>`
    );
  }

  if (executionHeader.next_pending?.title) {
    contextParts.push(
      `<span class="tower-exec-context-item"><span class="tower-exec-context-label">Next</span> ${escapeHtml(executionHeader.next_pending.title)}</span>`
    );
  }

  if (executionHeader.manifest_item_count != null) {
    contextParts.push(
      `<span class="tower-exec-context-item"><span class="tower-exec-context-label">Manifest</span> ${escapeHtml(String(executionHeader.manifest_item_count))} items</span>`
    );
  }

  if (executionHeader.live_only_row_count > 0) {
    contextParts.push(
      `<span class="tower-exec-context-item"><span class="tower-exec-context-label">Live only</span> ${escapeHtml(String(executionHeader.live_only_row_count))} unmatched</span>`
    );
  }

  if (executionHeader.refreshed_at_display) {
    contextParts.push(
      `<span class="tower-exec-context-item"><span class="tower-exec-context-label">Refreshed</span> ${escapeHtml(executionHeader.refreshed_at_display)} UTC</span>`
    );
  }

  const greenCount = executionHeader.green_row_count ?? 0;
  const greenNote =
    greenCount === 0
      ? `<span class="tower-exec-green-note tower-exec-green-zero">Green rows: 0 — no live worker running (expected when idle)</span>`
      : `<span class="tower-exec-green-note">Green rows: ${greenCount} — live claimed/running only</span>`;

  const contextHtml =
    contextParts.length > 0
      ? `<div class="tower-exec-context">${contextParts.join("")}${greenNote}</div>`
      : `<div class="tower-exec-context">${greenNote}</div>`;
  ...
  const motionNote =
    motionState === "stalled"
      ? `<span class="tower-stale-warn">Running long — check worker</span>`
      : motionState === "possibly_stale"
        ? `<span class="tower-stale-warn tower-stale-warn-soft">Possibly stalled — no recent update</span>`
        : tone === "running"
          ? `<span class="tower-running-meta muted">Ongoing · live</span>`
          : "";

  return `<div class="tower-execution-header tone-${escapeHtml(tone)}${motionClass}" role="status" aria-live="polite">
    <div class="tower-exec-headline">${headlineInner}</div>
    <div class="tower-exec-detail">${escapeHtml(executionHeader.detail || "")}${motionNote ? ` ${motionNote}` : ""}</div>
    ${contextHtml}
  </div>`;
}

function renderLiveSourceStatusHtml(liveSource, isLive) {
...
function renderCurrentQueueBlock(currentQueue, isLive, liveSource) {
  ...
  const liveSourceHtml = renderLiveSourceStatusHtml(liveSource, isLive);

  const liveOnlyBlock = `<div class="live-jobs-subsection">
    <h3>Live command-channel jobs <span class="count-pill">${liveOnlyRows.length}</span></h3>
    ${liveSourceHtml}
    <p class="muted live-jobs-note">Dynamic current/recent jobs from command channel — includes ad hoc work not in the batch plan · source: command-channel live</p>
    ${renderSingleBatchTable(liveOnlyRows, "(no live command-channel jobs right now)")}
  </div>`;
  ...
  const completedInfraBlock =
    ...
    <p class="muted completed-infra-note">Historical done items — batch/data-source scaffolding, not the active plan or table parent</p>
    ${renderSingleBatchTable(completedInfraRows, "(none)")}
  </details>`
      : "";

  const plannedBatchBlock = `<div class="planned-batch-section">
    <h2 class="tower-batch-container-title">${escapeHtml(currentQueue.label || "Tower Current Batch")} <span class="count-pill">${manifestRows.length}</span></h2>
    <p class="tower-batch-subtitle muted">${escapeHtml(batchName)} · ${escapeHtml(manifestPath)}${refreshNote}</p>
    <p class="tower-ia-explainer muted">Planned rows show the batch plan. Live command-channel jobs above show what is actually running.</p>
    <h3 class="planned-items-heading">Planned Batch Items <span class="count-pill">${activePlannedRows.length}</span></h3>
    <p class="muted planned-items-note">Manifest rows from ${escapeHtml(manifestPath)} — planned until a live job matches · source: planned or manifest + live</p>
    ${renderSingleBatchTable(activePlannedRows, "(no active planned items)")}
    ${completedInfraBlock}
  </div>`;
  ...
  return `<section class="batch-section batch-section-primary">
    ...
    ${plannedBatchBlock}
    ${reviewHistoryNote}
    <p class="batch-legend muted">Row colors: grey = Done · vivid green = Running/Ongoing (live claimed job only — pulsing dot + shimmer when active) · yellow = Waiting / Needs Tim / Awaiting approval · red = Failed · blue = Next / Queued / Not started · source markers: manifest + live (matched row) · command-channel live (dynamic unmatched row) · planned = no live job yet — static until linked · amber/orange = possibly stalled / running long</p>
  </section>`;
}
...
function renderTowerHtml(model) {
  ...
  const activityBlock = activityLog
    ? `<section class="activity-section activity-section-secondary">
  ...
  return renderShortStatusShell({
    title: "Control Tower",
    subtitle: "Current batch manifest — every planned item in one table, then activity history",
    badgeLabel,
    badgeClass,
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .tower-execution-header { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 0.95rem 1rem; margin-bottom: 0.85rem; border-left: 4px solid var(--running); }
    .tower-execution-header.tone-running { border-left-color: var(--batch-running); background: rgba(34, 197, 94, 0.08); }
    .tower-execution-header.tone-running.tower-exec-live-active { animation: tower-header-live-pulse 2.4s ease-in-out infinite; }
    ...
    .tower-execution-header.tone-running.tower-exec-live-possibly_stale {
      border-left-color: var(--batch-pending); background: rgba(251, 191, 36, 0.1); animation: none;
    }
    .tower-execution-header.tone-running.tower-exec-live-possibly_stale .tower-exec-headline-inner { color: #fde68a; }
    .tower-execution-header.tone-running.tower-exec-live-stalled {
      border-left-color: #fb923c; background: rgba(251, 146, 60, 0.12); animation: none;
    }
    .tower-execution-header.tone-running.tower-exec-live-stalled .tower-exec-headline-inner { color: #fdba74; }
    .tower-execution-header { position: relative; overflow: hidden; }
    .tower-execution-header.tone-idle { border-left-color: var(--batch-future); background: rgba(56, 189, 248, 0.08); }
    .tower-execution-header.tone-waiting { border-left-color: var(--batch-pending); background: rgba(251, 191, 36, 0.1); }
    .tower-execution-header.tone-failed { border-left-color: var(--batch-failed); background: rgba(248, 113, 113, 0.12); }
    .tower-execution-header.tone-done { border-left-color: var(--batch-completed); background: rgba(107, 114, 128, 0.12); }
    .tower-exec-headline { font-size: 1.2rem; font-weight: 700; line-height: 1.25; }
    .tower-exec-headline-inner { display: inline-flex; align-items: center; gap: 0.45rem; }
    .tower-execution-header.tone-running .tower-exec-headline { color: #86efac; }
    .tower-execution-header.tone-idle .tower-exec-headline { color: #bae6fd; }
    .tower-execution-header.tone-waiting .tower-exec-headline { color: #fde68a; }
    .tower-execution-header.tone-failed .tower-exec-headline { color: #fca5a5; }
    .tower-execution-header.tone-done .tower-exec-headline { color: #d1d5db; }
    ...
    .planned-batch-section { margin-top: 0.35rem; }
    .tower-batch-container-title { font-size: 1.05rem; margin: 0 0 0.25rem; display: flex; align-items: center; gap: 0.45rem; color: var(--text); font-weight: 650; }
    .tower-batch-subtitle { margin: 0 0 0.35rem; font-size: 0.82rem; }
    .tower-ia-explainer { margin: 0 0 0.75rem; font-size: 0.84rem; padding: 0.45rem 0.65rem; border-left: 3px solid var(--batch-future); background: rgba(56, 189, 248, 0.06); border-radius: 0 6px 6px 0; }
    .planned-items-heading { font-size: 0.92rem; margin: 0.45rem 0 0.35rem; display: flex; align-items: center; gap: 0.45rem; }
    ...
    .activity-section-secondary { opacity: 0.95; }
