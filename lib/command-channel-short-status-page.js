...
function deriveTowerStateLine(summary, timeSummary, motionState) {
  ...
  if (awaiting > 0) {
    return {
      state: "Needs Tim",
      detail: `${awaiting} job(s) awaiting approval`,
    };
  }
  if (failed > 0) {
    return {
  ...
  if (needsReview > 0) {
    return {
      state: "Needs Tim",
      detail: `${needsReview} completed job(s) need review`,
    };
  }
  if (stranded > 0) {
    return {
      state: "Waiting",
      detail: `${stranded} approved job(s) waiting for worker claim`,
    };
  }
  ...
  return {
    state: "Idle",
    detail: "No active command-channel work",
  };
}

function deriveTimActionItems(summary, timeSummary) {
  const items = [];
...
function buildCurrentQueue(summary, timeSummary, motionState, options = {}) {
  ...
  const stateLine = deriveTowerStateLine(summary, timeSummary, motionState);
  ...
  for (const item of reviewItems) {
    ...
    const entry = formatQueueJobEntry(classified, "review completed result");
...
function buildActivityLog(jobs, summary, timeSummary, motionState, options = {}) {
  ...
  const timActions = deriveTimActionItems(summary, timeSummary);
    ...
    <div class="queue-grid">
      <div class="queue-block">
        ...
        <div class="card"><table>
          ...
          <tbody>${renderQueueJobRows(currentQueue.now_running, "(none — workers idle)")}</tbody>
        </table></div>
      </div>
      <div class="queue-block">
        <h3>Needs Tim <span class="count-pill">${currentQueue.needs_tim.length}</span></h3>
        <div class="card"><table>
          <thead><tr><th>Job ID</th><th>Profile</th><th>Repo</th><th>Plan</th><th>Action</th></tr></thead>
          <tbody>${renderQueueJobRows(currentQueue.needs_tim, "(none — no Tim action needed)")}</tbody>
        </table></div>
      </div>
      <div class="queue-block">
...
function formatControlTowerReport(model) {
  ...
  if (model.current_queue) {
    ...
    renderQueueSection("Needs Tim", queue.needs_tim);
