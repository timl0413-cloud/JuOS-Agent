...
function renderCurrentQueueBlock(currentQueue, isLive, liveSource) {
  ...
  const liveOnlyInlineBlock =
    liveOnlyRows.length > 0
      ? `<div class="live-jobs-inline">
    <h3 class="live-jobs-inline-heading">Live command-channel jobs <span class="count-pill">${liveOnlyRows.length}</span></h3>
    ${renderSingleBatchTable(liveOnlyRows, "")}
  </div>`
      : "";
