...
const LANE_REGISTRY_V0 = [
  {
    ...
    lane_status: "active",
    launch_ready: true,
    note: "Station 1 JOA worker loop — primary TimOS-Agent lane",
  },
  {
    room: "Station 4 / 5",
    profile: "joa",
    ...
    launch_ready: false,
    note: "Worker hub panels planned; not always-on in v0",
  },
  {
    room: "NovaBridge (NB)",
    profile: "nova",
    lane_status: "pending",
    launch_ready: false,
    note: "Lane setup pending — Room Identity Card + Tim approval before launch",
  },
  {
    room: "JUB",
    profile: null,
    lane_status: "pending",
    launch_ready: false,
    note: "GSync publish/consume only until coordination lane is opened",
  },
  {
    room: "GSync",
    profile: "jucore",
  ...
  },
  {
    ...
    lane_status: "active",
    launch_ready: true,
    note: "Per-job repo_ref and workspace_ref required",
  },
  {
    room: "MinistryOps",
    profile: "ministry",
  ...
  },
  {
    ...
    lane_status: "active",
    launch_ready: true,
    note: "Finance worker hub must be running for claim",
  },
];
...
function formatLaneReadinessSummary(laneRegistry = LANE_REGISTRY_V0) {
  return laneRegistry
    .map((lane) => {
      const status = lane.launch_ready ? "ready" : lane.lane_status;
      const profile = lane.profile ? ` profile=${lane.profile}` : "";
      return `${lane.room}:${status}${profile}`;
    })
    .join("; ");
}
...
function renderTowerHtml(model) {
  ...
  const registryRows = (model.lane_registry || LANE_REGISTRY_V0)
    .map(
      (lane) => `<tr>
        ...
        <td>${escapeHtml(lane.profile || "—")}</td>
        <td><span class="lane-${escapeHtml(lane.lane_status)}">${escapeHtml(lane.lane_status)}</span></td>
  ...
  const body = `
    ...
    <section>
      <h2>Cross-room launch readiness</h2>
      <p class="muted" style="margin: 0 0 0.65rem;">${escapeHtml(model.lane_readiness_summary || "")}</p>
      <div class="card"><table>
        <thead><tr><th>Room</th><th>Profile</th><th>Lane status</th><th>Launch ready</th><th>Note</th></tr></thead>
...
function renderLocalLiveLandingHtml(options = {}) {
  ...
  const reasonText =
    reason === "auth_not_configured"
      ? "This server process does not have command-channel auth loaded."
      : "Live data requires local setup or a Bearer token.";
...
<head>
  ...
  <title>${escapeHtml(title)} — live access</title>
  <style>
    :root { --bg: #0f1419; --surface: #1a2332; --text: #e7ecf3; --muted: #9aa8bc; --border: #334155; --running: #38bdf8; --proto: #a78bfa; --live: #34d399; --sans: system-ui, sans-serif; --mono: "Cascadia Code", Consolas, monospace; }
    ...
    .badge { display: inline-block; font-size: 0.72rem; font-weight: 600; letter-spacing: 0.04em; text-transform: uppercase; padding: 0.2rem 0.55rem; border-radius: 999px; border: 1px solid rgba(167,139,250,0.35); background: rgba(167,139,250,0.15); color: var(--proto); }
...
<body>
  <div class="wrap">
    ...
    <p class="subtitle">Live view — local browser setup required</p>
    <span class="badge">Not live — setup required</span>
    <div class="card">
      <h2>Do not confuse with preview</h2>
      <p><a href="/${escapeHtml(route)}/preview"><strong>/${escapeHtml(route)}/preview</strong></a> is <span class="warn">SAMPLE DATA ONLY</span> — fake jobs for layout testing.</p>
      <p>This page is for <strong>LIVE DATA</strong> from the real command-channel store.</p>
    </div>
    <div class="card">
    ...
    <div class="card">
      <h2>Local live browser (recommended on Station 1 / 4 / 5)</h2>
      <ol>
        <li>Start the server from a <strong>token-loaded supervisor shell</strong> (auth already loaded in that session).</li>
        <li>Open the live URL in your browser — no Bearer header needed on 127.0.0.1.</li>
      </ol>
      <span class="cmd">npm run server:command-channel</span>
      <span class="cmd">start http://127.0.0.1:8790/${escapeHtml(route)}</span>
    </div>
    <div class="card">
