...
function buildCurrentQueueFromManifest(manifest, summary, timeSummary, motionState, options = {}) {
  ...
  if (activeCount === 0 && completedItems.length > 0) {
  ...
  return {
    ...
    label: `Current batch — ${batchName}`,
    note: `${manifestNotes} · ${manifestPathLabel} · planned rows stay static until a live job matches; live rows show · live and sync from command-channel state · green = matched claimed/running job only`,
...
function renderBatchTaskRows(entries, emptyText) {
  ...
  return entries
    .map((row) => {
      ...
      const sourceClass = row.live_linked ? "batch-live-linked" : "batch-planned";
      const sourceHint = row.live_linked
        ? "Linked to live command-channel job"
      ...
      return `<tr class="batch-row batch-state-${escapeHtml(batchState)} ${sourceClass}${motionClass}" title="${escapeHtml(sourceHint)}">
        ...
        <td class="batch-meta muted">${escapeHtml(row.profile)} · ${escapeHtml(row.repo)}</td>
      </tr>`;
    })
    .join("");
}
...
function renderCurrentQueueBlock(currentQueue, isLive) {
  ...
  const batchTable = renderSingleBatchTable(
    batchRows,
    "(no jobs in current batch view — idle or no recent command-channel activity)"
  );
  const rowCount = batchRows.length;

  return `<section class="batch-section batch-section-primary">
    ${executionHeaderHtml}
    <h2>${escapeHtml(currentQueue.label)} <span class="count-pill">${rowCount}</span></h2>
    <p class="muted queue-note">${escapeHtml(currentQueue.note)}${isLive ? " · auto-refresh 30s" : ""}</p>
    ${reviewHistoryNote}
    ${batchTable}
    <p class="batch-legend muted">Row colors: grey = Done · vivid green = Running/Ongoing (live claimed job only — pulsing dot + shimmer when active) · yellow = Waiting / Needs Tim / Awaiting approval · red = Failed · blue = Next / Queued / Not started · “· live” = matched command-channel job · “· planned” = no live job yet — static until linked · amber/orange = possibly stalled / running long</p>
  </section>`;
}
...
function formatControlTowerReport(model) {
  ...
  if (model.current_queue) {
    ...
    lines.push("", "Batch rows (completed → current → future):");
    if (batchRows.length === 0) {
      ...
      for (const row of batchRows) {
        ...
        lines.push(
          `[${row.batch_state || "pending"}] ${row.id} | ${row.plan} | ${row.purpose || "—"} | ${row.status_label || row.hint || row.status}${timAction} | ${row.profile} · ${row.repo}`
        );
      }
    }
  }
