...
function buildCurrentQueue(summary, timeSummary, motionState, options = {}) {
  ...
  const nowRunning = summary.buckets[STATUS_CATEGORIES.CLAIMED_RUNNING].map((item) =>
    formatQueueJobEntry(item, item.job.claimed_by ? `claimed by ${item.job.claimed_by}` : "running")
  );

  const needsTim = [];
  for (const item of summary.buckets[STATUS_CATEGORIES.PENDING_AWAITING_APPROVAL]) {
    const entry = formatQueueJobEntry(item, "awaiting Tim approval");
    if (entry) {
      needsTim.push(entry);
    }
  }
  for (const item of summary.buckets[STATUS_CATEGORIES.PENDING_UNCLAIMED]) {
    const entry = formatQueueJobEntry(
      item,
      `start ${item.job.target_worker_profile || "worker"} loop to claim`
    );
    if (entry) {
      needsTim.push(entry);
    }
  ...
  for (const item of summary.buckets[STATUS_CATEGORIES.FAILED_OR_BLOCKED]) {
    const err = item.job.errors?.length
      ...
      : item.reason || "blocked";
    const entry = formatQueueJobEntry(item, `resolve: ${err}`);
    if (entry) {
      needsTim.push(entry);
    }
  }
  const runningTooLongItems =
    timeSummary?.watchlist?.buckets[WATCHLIST_CATEGORIES.RUNNING_TOO_LONG] || [];
  for (const item of runningTooLongItems) {
    const classified = item.classified || item;
    const entry = formatQueueJobEntry(classified, "running longer than expected");
    if (entry && !needsTim.some((row) => row.id === entry.id)) {
      needsTim.push(entry);
    }
  }
  for (const item of reviewSplit.urgent) {
    const classified = item.classified || item;
    const entry = formatQueueJobEntry(classified, "review completed result · recent");
    if (entry && !needsTim.some((row) => row.id === entry.id)) {
      needsTim.push(entry);
    }
  }

  const nextUp = sortQueueEntriesByAt(
    summary.buckets[STATUS_CATEGORIES.PENDING_UNCLAIMED].map((item) =>
      formatQueueJobEntry(item, "approved · waiting for worker claim")
    ).filter(Boolean)
  );

  const recentlyCompleted = sortQueueEntriesByAt(
    [
      ...summary.buckets[STATUS_CATEGORIES.COMPLETED].map((item) => {
        const hint = item.job.result?.summary
          ...
          : "completed";
        return formatQueueJobEntry(item, hint);
      }),
      ...summary.buckets[STATUS_CATEGORIES.FAILED_OR_BLOCKED].map((item) => {
        const err = item.job.errors?.length
          ? truncateText(item.job.errors.join("; "), 72)
          : item.reason || item.job.status;
        return formatQueueJobEntry(item, err);
      }),
    ].filter(Boolean),
    "desc"
  ).slice(0, maxRecent);
  ...
  const reviewHistory = sortQueueEntriesByAt(
    reviewSplit.historical
      .map((item) => {
        const classified = item.classified || item;
        return formatQueueJobEntry(
          classified,
          "review history · not a blocker"
        );
      })
      .filter(Boolean),
    "desc"
  ...
  if (activeCount === 0 && recentlyCompleted.length === 0 && reviewHistory.length === 0) {
    idleMessage = "No known jobs — idle";
  } else if (activeCount === 0 && actionable.count === 0) {
    idleMessage =
      reviewHistory.length > 0
        ? "No workers running · no approvals pending · review history below is for reference only"
        : "No workers running · no approvals pending · no approved jobs waiting";
  }

  return {
    source: "reconstructed_from_jobs",
    label: "Current Queue v0",
...
function renderCurrentQueueBlock(currentQueue, isLive) {
  ...
  const idleNote = currentQueue.idle_message
    ...
    : "";
  const reviewHistory = currentQueue.review_history || [];
  const reviewHistoryTotal =
    currentQueue.review_history_total ?? reviewHistory.length;
  const reviewHistoryNote =
    reviewHistoryTotal > reviewHistory.length
      ? `<p class="muted queue-history-note">Showing ${reviewHistory.length} of ${reviewHistoryTotal} older completed jobs · not blockers</p>`
      : reviewHistory.length > 0
        ? `<p class="muted queue-history-note">Older completed jobs for reference · not blockers</p>`
        : "";
  ...
  return `<section class="queue-section">
    ...
    <div class="queue-grid">
      <div class="queue-block">
        <h3>Now running <span class="count-pill">${currentQueue.now_running.length}</span></h3>
        <div class="card"><table>
          <thead><tr><th>Job ID</th><th>Profile</th><th>Repo</th><th>Plan</th><th>Status</th></tr></thead>
          <tbody>${renderQueueJobRows(currentQueue.now_running, "(none — no workers running)")}</tbody>
        </table></div>
      </div>
      <div class="queue-block">
        <h3>Needs attention <span class="count-pill">${currentQueue.needs_tim.length}</span></h3>
        <p class="muted queue-block-note">Awaiting approval, blocked, stranded, or recent review</p>
        <div class="card"><table>
          <thead><tr><th>Job ID</th><th>Profile</th><th>Repo</th><th>Plan</th><th>Action</th></tr></thead>
          <tbody>${renderQueueJobRows(currentQueue.needs_tim, "(none — no actionable items)")}</tbody>
        </table></div>
      </div>
      <div class="queue-block">
        <h3>Next up <span class="count-pill">${currentQueue.next_up.length}</span></h3>
        <div class="card"><table>
          <thead><tr><th>Job ID</th><th>Profile</th><th>Repo</th><th>Plan</th><th>Status</th></tr></thead>
          <tbody>${renderQueueJobRows(currentQueue.next_up, "(none — no approved jobs waiting)")}</tbody>
        </table></div>
      </div>
      <div class="queue-block">
        <h3>Recently completed <span class="count-pill">${currentQueue.recently_completed.length}</span></h3>
        <div class="card"><table>
          <thead><tr><th>Job ID</th><th>Profile</th><th>Repo</th><th>Plan</th><th>Result</th></tr></thead>
          <tbody>${renderQueueJobRows(currentQueue.recently_completed, "(none recently)")}</tbody>
        </table></div>
      </div>
      <div class="queue-block">
        <h3>Review history <span class="count-pill">${reviewHistory.length}</span></h3>
        ${reviewHistoryNote}
        <div class="card"><table>
          <thead><tr><th>Job ID</th><th>Profile</th><th>Repo</th><th>Plan</th><th>Note</th></tr></thead>
          <tbody>${renderQueueJobRows(reviewHistory, "(none — no older completed jobs)")}</tbody>
        </table></div>
      </div>
    </div>
  </section>`;
...
function renderTowerHtml(model) {
  ...
  return renderShortStatusShell({
    title: "Control Tower",
    subtitle: "Current queue + activity log — what is running, what needs Tim, what is next",
    badgeLabel,
    badgeClass,
...
function formatControlTowerReport(model) {
  ...
  if (model.current_queue) {
    ...
    const renderQueueSection = (title, entries) => {
      lines.push("", `${title}:`);
      if (!entries || entries.length === 0) {
        lines.push("(none)");
        return;
      ...
      for (const row of entries) {
        lines.push(
          `${row.id} | ${row.profile} | ${row.repo} | ${row.plan} | ${row.hint || row.status}`
        );
      }
    };
    ...
    renderQueueSection("Now running", queue.now_running);
    renderQueueSection("Needs attention", queue.needs_tim);
    renderQueueSection("Next up", queue.next_up);
    renderQueueSection("Recently completed", queue.recently_completed);
    renderQueueSection("Review history", queue.review_history);
  }
