...
function buildCurrentQueueFromManifest(manifest, summary, timeSummary, motionState, options = {}) {
  ...
  return {
    ...
    note: `Explicit manifest · ${manifestPathLabel} · linked command-channel jobs override row status`,
...
function renderBatchTaskRows(entries, emptyText) {
  ...
  return entries
    .map((row) => {
      ...
      return `<tr class="batch-row batch-state-${escapeHtml(batchState)}">
...
function renderCurrentQueueBlock(currentQueue, isLive) {
  ...
  return `<section class="batch-section batch-section-primary">
    ...
    ${reviewHistoryNote}
    ${batchTable}
    <p class="batch-legend muted">Row colors: grey = completed · green = running · yellow = pending / needs action · red = failed / blocked · blue = future / not started</p>
  </section>`;
}
...
function renderTowerHtml(model) {
  ...
  return renderShortStatusShell({
    title: "Control Tower",
    subtitle: "One batch task table — status at a glance, then activity history",
    badgeLabel,
    badgeClass,
...
function formatControlTowerReport(model) {
  ...
  if (model.current_queue) {
    ...
    lines.push(
      ...
      queue.note ||
        (queue.source === "explicit_manifest"
          ? "Explicit current batch manifest"
