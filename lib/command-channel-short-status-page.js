...
function buildTowerSummary(jobs, options = {}) {
  ...
  return {
    ...
    generated_at: options.generated_at || new Date().toISOString(),
    motion: {
      system_state: deriveSystemState(summary, timeSummary),
...
function renderTowerHtml(model) {
  ...
  return renderShortStatusShell({
    ...
    body,
    footerNote:
      "CLI equivalent: npm run status:tower · Doc: docs/command-channel/short-status-routes-v0.md",
  });
}
...
function formatControlTowerReport(model) {
  const lines = [
    ...
    `Swept: ${model.swept_at}`,
    `System: ${model.motion.system_state}`,
    `Running: ${model.motion.running_count} | Stranded: ${model.motion.stranded_count} | Needs attention: ${model.motion.needs_attention_count}`,
    "",
    `Next owner: ${model.next_up.owner}`,
    `Next action: ${model.next_up.action}`,
  ];
