...
const { ageMinutesSince } = require("./command-channel-time-sweep");
...
const UNMATCHED_LIVE_CATEGORY_PRIORITY = {
...
const UNMATCHED_RECENT_MAX_AGE_MINUTES = 120;
...
function classifiedStatusLabel(classified, options = {}) {
  ...
  if (category === STATUS_CATEGORIES.CLAIMED_RUNNING) {
    if (options.runningTooLong) {
      return job.claimed_by ? `Running long · ${job.claimed_by} · live` : "Running long · live";
    }
    return "Running · live";
  }
  if (category === STATUS_CATEGORIES.PENDING_AWAITING_APPROVAL) {
    return "Awaiting approval · live";
  }
  if (category === STATUS_CATEGORIES.PENDING_UNCLAIMED) {
    return "Queued / pending live";
  }
...
function resolveManifestItemRow(manifestItem, context) {
  ...
  if (linked?.job?.id) {
    ...
    if (entry) {
      ...
      entry.link_hint = `Linked job: ${job.id}`;
      entry.status_source = "manifest + live";
...
function collectManifestMatchedJobIds(manifest, jobIndex) {
  const matched = new Set();
  for (const item of manifest?.items || []) {
    const { classified } = resolveManifestLinkedJob(item, jobIndex);
    if (classified?.job?.id) {
      matched.add(classified.job.id);
    }
  }
  return matched;
}

function isUnmatchedJobRelevant(classified, options = {}) {
  const category = classified?.category;
  const job = classified?.job;
  if (!job?.id) {
    return false;
  ...
  const recentMaxMinutes =
    options.recent_max_age_minutes ?? UNMATCHED_RECENT_MAX_AGE_MINUTES;
  ...
  if (
    ...
    const at =
      ...
      job.approved_at ||
      job.created_at;
    const ageMinutes = ageMinutesSince(at, nowMs);
    return ageMinutes != null && ageMinutes <= recentMaxMinutes;
  }

  return false;
}

function findUnmatchedRelevantJobs(manifest, jobIndex, options = {}) {
  const matchedIds = collectManifestMatchedJobIds(manifest, jobIndex);
  const unmatched = [];

  for (const classified of jobIndex.values()) {
    if (!classified?.job?.id || matchedIds.has(classified.job.id)) {
      continue;
    }
    if (isUnmatchedJobRelevant(classified, options)) {
      unmatched.push(classified);
    }
  }

  return unmatched.sort((a, b) => {
    const aPriority = UNMATCHED_LIVE_CATEGORY_PRIORITY[a.category] || 0;
    const bPriority = UNMATCHED_LIVE_CATEGORY_PRIORITY[b.category] || 0;
    if (aPriority !== bPriority) {
      return bPriority - aPriority;
    }
    return jobRecencyMs(b) - jobRecencyMs(a);
...
function formatUnmatchedLiveJobRow(classified, context) {
  const {
    runningTooLongIds,
    urgentReviewIds,
    firstStrandedJobId,
  ...
  const urgentReview = urgentReviewIds.has(job.id);
  const runningTooLong = runningTooLongIds.has(job.id);
  const liveStatusLabel = classifiedStatusLabel(classified, {
    urgentReview,
    runningTooLong,
  });
  const isRunning = classified.category === STATUS_CATEGORIES.CLAIMED_RUNNING;
  const statusLabel = liveStatusLabel;
  const timAction = classifiedTimAction(classified, {
    urgentReview,
    runningTooLong,
    firstStranded: firstStrandedJobId === job.id,
  });
  const liveBatchState = isRunning
    ? BATCH_STATES.RUNNING
    : classifiedBatchState(classified.category, BATCH_STATES);

  const entry = formatBatchTaskEntry(classified, {
    batch_state: liveBatchState,
    status_label: statusLabel,
    hint: statusLabel,
    tim_action_needed: timAction,
  ...
  entry.plan = jobTitleFn ? jobTitleFn(job) : job.plan_summary || job.id;
  entry.purpose = job.approval_summary || job.task_type || "—";
  entry.live_linked = isRunning;
  entry.link_hint = `Linked job: ${job.id}`;
  entry.status_source = "command-channel live";
  entry.row_source = "dynamic_live";
  entry.manifest_matched = false;
...
function buildUnmatchedLiveJobRows(manifest, jobIndex, context, options = {}) {
  return findUnmatchedRelevantJobs(manifest, jobIndex, options)
    .map((classified) => formatUnmatchedLiveJobRow(classified, context))
    .filter(Boolean);
}
...
module.exports = {
  ...
  collectManifestMatchedJobIds,
  isUnmatchedJobRelevant,
  findUnmatchedRelevantJobs,
  formatUnmatchedLiveJobRow,
